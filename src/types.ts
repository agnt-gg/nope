...
// ── Tool Types (compatible with @agnt-gg/ai, @agnt-gg/plug, @agnt-gg/flow) ──
...
export type Param =
  | Scalar
  | `${Scalar}?`
  | { type: Scalar; description?: string; required?: boolean; default?: unknown };
...
export interface SecurityReport {
  ...
  /** Per-tool breakdown */
  perTool: Record<string, { checks: number; blocks: number }>;
}
...
export interface RedTeamResult {
  ...
  coverage: Record<string, { tested: number; caught: number }>;
}
