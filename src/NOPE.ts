...
export class NOPE {
  ...
  sandbox(config?: SandboxConfig): Sandbox {
    ...
    if (backend === 'docker') return this._dockerSandbox(config ?? {});
    if (backend === 'ssh') return this._sshSandbox(config ?? {});
    if (backend === 'wasm') return this._wasmSandbox(config ?? {});
    ...
    return {
      exec: async (command: string): Promise<SandboxResult> => {
        ...
        try {
          ...
          if (config.tmpfs) for (const t of config.tmpfs) args.push('--tmpfs', t);
  ...
  private _wasmSandbox(config: SandboxConfig): Sandbox {
    const timeout = config.timeout ?? 30000;
