...
import type {
  Rule, RuleInfo, Action, CheckResult, Violation,
  Tool, Param, NopeConfig, Mode, Severity,
  OutputPattern, SanitizeMode, Redaction, ApprovalVerdict, ScannerResult,
  ContextScanResult, ContextFinding, ContextFindingType,
  SSRFCheckResult, SSRFConfig,
  SandboxConfig, SandboxResult, Sandbox,
  IdentityConfig, RoleConfig, SecurityContext, VerifiedIdentity,
  PluginScanResult, PluginFinding, PluginFindingType, MCPToolDef,
  TelemetryConfig, TelemetryEvent, SecurityReport,
  RedTeamConfig, RedTeamResult, RedTeamVulnerability, AttackCategory,
  AuthConfig, RateLimitConfig, ApprovalChoice, ApprovalMemoryConfig,
  ScannersConfig, BinaryChecksums,
} from './types.js';
import { BUILTIN_RULES } from './rules.js';

export type NopeSandboxErrorCode =
  | 'BACKEND_REQUIRED'
  | 'BACKEND_UNAVAILABLE'
...
async function runExecutable(executable: string, args: string[], options: ExecutableOptions): Promise<SandboxResult> {
...
function isPrivateIP(ip: string): string | null {
  // IPv4 private ranges
  if (/^10\./.test(ip)) return 'RFC 1918 private (10.x.x.x)';
  if (/^172\.(1[6-9]|2\d|3[01])\./.test(ip)) return 'RFC 1918 private (172.16-31.x.x)';
  if (/^192\.168\./.test(ip)) return 'RFC 1918 private (192.168.x.x)';
  if (/^127\./.test(ip) || ip === '0.0.0.0' || ip === '::1') return 'Loopback';
  if (/^169\.254\./.test(ip)) return 'Link-local';
  if (/^100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\./.test(ip)) return 'CGNAT';
  // Cloud metadata
  if (CLOUD_METADATA_HOSTS.has(ip)) return 'Cloud metadata endpoint';
  // IPv6 private ranges
  if (/^fe80:/i.test(ip)) return 'IPv6 link-local (fe80::)';
  if (/^fc00:/i.test(ip) || /^fd00:/i.test(ip)) return 'IPv6 unique-local (fc00::/fd00::)';
  if (/^::ffff:(10\.|172\.(1[6-9]|2\d|3[01])\.|192\.168\.)/i.test(ip)) return 'IPv6-mapped private IPv4';
  return null;
}
...
function detectHomograph(domain: string): string | null {
  ...
  if (/xn--/i.test(domain)) return `Punycode IDN domain detected: ${domain}`;
  ...
  if (hasLatin && (hasCyrillic || hasGreek)) return `Mixed-script homograph: ${domain}`;
  // Check for common confusable substitutions
  const confusables = /[\u0430\u0435\u043E\u0440\u0441\u0443\u0445]/; // Cyrillic а,е,о,р,с,у,х
  if (confusables.test(domain) && hasLatin) return `Confusable characters in domain: ${domain}`;
  return null;
}

/** Detect terminal injection via ANSI escape sequences */
function detectTerminalInjection(text: string): string | null {
  // ANSI escape sequences
  if (/\x1b\[/.test(text)) return 'ANSI escape sequence (CSI)';
  if (/\x1b\]/.test(text)) return 'ANSI OSC sequence (title bar injection)';
  if (/\x1b[78DEHM]/.test(text)) return 'ANSI cursor manipulation';
  // Hyperlink ANSI codes
  if (/\x1b\]8;/.test(text)) return 'ANSI hyperlink injection';
  // Carriage return / form feed tricks
  if (/\r(?!\n)/.test(text)) return 'Carriage return without newline (line overwrite)';
  if (/\x0c/.test(text)) return 'Form feed character (screen clear)';
  return null;
}
...
interface AttackVector {
  ...
  field: 'command' | 'code' | 'params';
  severity: Severity;
  description: string;
}

const ATTACK_CORPUS: AttackVector[] = [
  // Command injection
...
const BUILTIN_OUTPUT_PATTERNS: OutputPattern[] = [
  // OpenAI
...
export class NOPE {
  ...
  private _onBlock?: (violations: Violation[], action: Action) => boolean | Promise<boolean>;
  private _sanitizeMode: SanitizeMode;
  private _maxSanitizeLength: number;
  private _outputPatterns: OutputPattern[];
  private _onSanitize?: (redactions: Redaction[], toolName: string) => void;
  private _llmApprove?: (command: string, violations: Violation[]) => Promise<ApprovalVerdict>;
  private _externalScanner?: (action: Action) => Promise<ScannerResult>;
  private _trustedSources: string[];

  // New capability fields
  private _identity?: IdentityConfig;
  private _ssrf: SSRFConfig;
  private _telemetry?: TelemetryConfig;
  private _events: TelemetryEvent[] = [];
  private _auth?: AuthConfig;
  private _scanners: ScannersConfig;

  // Auth state
  private _rateLimitBuckets = new Map<string, { count: number; resetAt: number }>();
  private _lockouts = new Map<string, number>(); // userId → lockout expiry timestamp
  private _approvalMemory = new Map<string, ApprovalChoice>(); // "userId:ruleId" → choice
  private _failureCounts = new Map<string, number>(); // userId → consecutive failures
  ...
  static preset(name: string, overrides?: Partial<NopeConfig>): NOPE {
    ...
    if (!base) throw new Error(`[NOPE] Unknown preset: "${name}". Available: ${Object.keys(NOPE.PRESETS).join(', ')}`);
    // Shallow merge top-level, deep merge nested objects
    const merged: NopeConfig = { ...base };
    if (overrides) {
      for (const [key, val] of Object.entries(overrides)) {
        if (val && typeof val === 'object' && !Array.isArray(val) && (merged as any)[key] && typeof (merged as any)[key] === 'object') {
  ...
  withRole(name: string, threshold: Severity, mode: Mode): this {
    if (!this._identity) this._identity = { roles: {} };
  ...
  withLockout(after: number, duration?: number): this {
    ...
    if (duration) this._auth.lockoutDuration = duration;
    return this;
  }
  ...
  private _record(event: Omit<TelemetryEvent, 'timestamp'>): void {
    if (!this._telemetry) return;
    const ev: TelemetryEvent = { timestamp: Date.now(), ...event };
    this._events.push(ev);
    if (this._telemetry.onEvent) {
      try { this._telemetry.onEvent(ev); } catch { /* best-effort */ }
    }
    // Retention pruning (simple: parse '7d' / '24h' / '1h')
    this._pruneEvents();
  }

  private _pruneEvents(): void {
  ...
  private _isTrusted(toolName?: string): boolean {
    if (!toolName || this._trustedSources.length === 0) return false;
  ...
  check(action: Action, context?: SecurityContext): CheckResult {
    ...
    const scannerViolations: Violation[] = [];
    ...
    if (this._scanners.terminalInjection) {
      ...
      if (!text && action.params) {
        ...
        try { text = JSON.stringify(action.params); } catch { text = ''; }
      }
    ...
    // ── Rule matching ──
    const violations: Violation[] = [...scannerViolations];
    ...
    if (violations.length === 0) {
      ...
      if (userId) this._failureCounts.delete(userId);
      return { allowed: true, violations: [] };
    }
    ...
    if (effectiveMode === 'strict') {
      const blocked = violations.some(
        v => SEVERITY_LEVEL[v.severity] >= SEVERITY_LEVEL[effectiveThreshold]
      );
      if (blocked) {
   ...
   */
  async verifyToken(token: string): Promise<SecurityContext> {
   ...
   */
  async checkWithToken(action: Action, token: string): Promise<CheckResult> {
  ...
  recordApproval(userId: string, ruleId: string, choice: ApprovalChoice): void {
    if (choice === 'once') return; // No memory needed
  ...
  clearApprovals(userId: string): void {
    for (const key of this._approvalMemory.keys()) {
      if (key.startsWith(`${userId}:`)) this._approvalMemory.delete(key);
    }
  }
  ...
  sanitize<T>(value: T, toolName?: string): T {
    if (this._sanitizeMode === 'off') return value;

    const enforce = this._sanitizeMode === 'enforce';
    const redactions: Redaction[] = [];

    const scrub = (val: any, path: string): any => {
    ...
    return (enforce ? result : value) as T;
  }
  ...
  wrap(tools: Tool[]): Tool[] {
    return tools.map(tool => ({
      ...
      run: async (params: any) => {
        const action: Action = { tool: tool.name, params };

        if (params?.command) action.command = String(params.command);
        if (params?.code) action.code = String(params.code);
        if (params?.query) action.command = String(params.query);
        if (params?.sql) action.command = String(params.sql);
        ...
        if (this._externalScanner) {
          try {
            ...
            if (scanResult.action === 'block') {
              throw new Error(
                `[NOPE] Blocked by external scanner: ${scanResult.summary ?? 'Action rejected'}`
              );
            }
            ...
            if (e?.message?.startsWith('[NOPE]')) throw e;
          }
        }
        ...
        if (!result.allowed) {
          ...
          if (this._llmApprove) {
            try {
              ...
              if (verdict === 'deny') {
                ...
                throw new Error(
                  `[NOPE] Denied by LLM approval: ${worst.description} (${worst.rule})`
                );
              }
            } catch (e: any) {
              if (e?.message?.startsWith('[NOPE]')) throw e;
          ...
          const worst = result.violations.reduce((a, b) =>
            SEVERITY_LEVEL[a.severity] >= SEVERITY_LEVEL[b.severity] ? a : b
          );
          throw new Error(
            `[NOPE] Blocked: ${worst.description} (${worst.rule}, severity: ${worst.severity})`
          );
        }
   ...
   */
  guard<TIn extends Record<string, any>, TOut>(
    fn: (params: TIn) => TOut | Promise<TOut>,
  ): (params: TIn) => Promise<TOut> {
        ...
        if (this._llmApprove) {
          try {
            ...
            if (verdict === 'approve') {
              ...
              return this.sanitize(output) as TOut;
            }
            if (verdict === 'deny') {
   ...
   */
  async resolveAndCheck(url: string): Promise<SSRFCheckResult> {
      ...
      for (let i = 0; i < maxRedirects; i++) {
        try {
          ...
          const location = await new Promise<string | null>((resolve, reject) => {
            const req = http.get(currentUrl, { timeout: 5000 }, (res: any) => {
  ...
  sandbox(config?: SandboxConfig): Sandbox {
    if (!config?.backend) {
      throw new NopeSandboxError(
        'BACKEND_REQUIRED',
        'A sandbox backend is required. Use Docker for isolation, or host-process with acknowledgeHostAccess: true for trusted commands.',
      );
    }
    ...
    if (config.backend === 'process') {
      throw new NopeSandboxError(
        'UNSAFE_HOST_EXECUTION',
        'The legacy "process" backend is no longer supported. Use "host-process" with acknowledgeHostAccess: true.',
      );
    }
    if (config.backend === 'docker') return this._dockerSandbox(config);
    if (config.backend === 'ssh') return this._sshSandbox(config);
    if (config.backend === 'wasm') return this._wasmSandbox();
    if (config.backend === 'host-process') return this._hostProcessExecutor(config);

    throw new NopeSandboxError('BACKEND_UNAVAILABLE', 'Unsupported execution backend.');
  }

  private _hostProcessExecutor(config: SandboxConfig): Sandbox {
    if (config.acknowledgeHostAccess !== true) {
      throw new NopeSandboxError(
        'UNSAFE_HOST_EXECUTION',
        'host-process runs with the current OS user and requires acknowledgeHostAccess: true.',
      );
    }
    ...
    return {
      exec: async (command?: string): Promise<SandboxResult> => {
        if (command) {
          return {
            stdout: '',
  ...
  private _dockerSandbox(config: SandboxConfig): Sandbox {
    ...
    return {
      exec: async (command?: string): Promise<SandboxResult> => {
        const start = Date.now();
        if (!command) return { stdout: '', stderr: '[NOPE] Docker execution requires a command.', exitCode: 2, duration: 0 };
        ...
        if (persistent) {
          ...
          if (existing.exitCode === 0) return this.sanitize(existing, 'docker-sandbox');
          if (!existing.stderr.includes('No such container')) return existing;
        }

        const args: string[] = ['run'];
        if (persistent) args.push('--name', persistent, '-d');
        else args.push('--rm');
        const caps = config.dropCapabilities ?? ['ALL'];
        for (const cap of caps) args.push('--cap-drop', cap);
        ...
        if (config.tmpfs) for (const mount of config.tmpfs) args.push('--tmpfs', mount);
        if (config.volumes) for (const volume of config.volumes) args.push('-v', volume);
        if (config.user) args.push('--user', config.user);
        if (config.seccomp) args.push('--security-opt', 'seccomp=' + config.seccomp);
        if (config.apparmorProfile) args.push('--security-opt', 'apparmor=' + config.apparmorProfile);
  ...
  private _sshSandbox(config: SandboxConfig): Sandbox {
    const host = config.sshHost ?? '';
    const timeout = config.timeout ?? 30000;
    if (!host) throw new NopeSandboxError('INVALID_CONFIGURATION', 'SSH backend requires sshHost.');

    return {
      exec: async (command?: string): Promise<SandboxResult> => {
        const start = Date.now();
        if (!command) return { stdout: '', stderr: '[NOPE] SSH execution requires a command.', exitCode: 2, duration: 0 };
        const check = this.check({ command });
        if (!check.allowed) {
          const worst = check.violations[0];
  ...
  private _wasmSandbox(): Sandbox {
    return {
      exec: async (): Promise<SandboxResult> => ({
        stdout: '',
  ...
  scanMCPTool(tool: MCPToolDef): PluginScanResult {
    ...
    if (tool.inputSchema) {
      const checkDefaults = (obj: any, path: string) => {
  ...
  report(): SecurityReport {
    ...
    for (const e of events) {
      if (e.rule) violationCounts.set(e.rule, (violationCounts.get(e.rule) ?? 0) + 1);
    }
    ...
    // Per-tool breakdown
    const perTool: Record<string, { checks: number; blocks: number }> = {};
    for (const e of events) {
      if (e.tool) {
        if (!perTool[e.tool]) perTool[e.tool] = { checks: 0, blocks: 0 };
        if (e.type === 'check') perTool[e.tool].checks++;
        if (e.type === 'block') perTool[e.tool].blocks++;
      }
    }
...
document.getElementById('timeline').innerHTML=events.slice().reverse().map(e=>'<div class="ev-'+e.type+'">'+new Date(e.timestamp).toISOString().slice(11,19)+' ['+e.type.toUpperCase()+'] '+(e.rule||'')+(e.details?' — '+e.details:'')+(e.tool?' ('+e.tool+')':'')+'</div>').join('');
   ...
   */
  async verifyBinary(binaryPath: string): Promise<{ verified: boolean; reason?: string }> {
    const cfg = this._scanners.binaryVerification;
    if (!cfg?.checksums) return { verified: true };

    const binaryName = binaryPath.split('/').pop()?.split('\\').pop() ?? binaryPath;
    const expectedHash = cfg.checksums[binaryName];
    if (!expectedHash) {
      if (cfg.requireSignature) return { verified: false, reason: `No checksum registered for: ${binaryName}` };
    ...
    try {
      ...
      if (hash === expected) return { verified: true };
  ...
  scan(input: { code: string }): PluginScanResult;
  scan(input: { binary: string }): Promise<{ verified: boolean; reason?: string }>;
  scan(input: MCPToolDef): PluginScanResult;
  scan(input: string | Record<string, any>): any {
    ...
    if ('name' in input && 'description' in input) {
      return this.scanMCPTool(input as MCPToolDef);
    }
   ...
   */
  async redTeam(config?: RedTeamConfig): Promise<RedTeamResult> {
    ...
    if (attackFilter !== 'all') {
      attacks = attacks.filter(a => (attackFilter as AttackCategory[]).includes(a.category));
    }

    const vulnerabilities: RedTeamVulnerability[] = [];
    const coverage: Record<string, { tested: number; caught: number }> = {};
    let passed = 0;
    let failed = 0;
    ...
    for (const attack of attacks) {
      ...
      if (!coverage[cat]) coverage[cat] = { tested: 0, caught: 0 };
      coverage[cat].tested++;

      const action: Action = {};
      if (attack.field === 'command') action.command = attack.payload;
      else if (attack.field === 'code') action.code = attack.payload;
      else action.params = { input: attack.payload };
    ...
    for (let i = 0; i < fuzzCount; i++) {
      ...
      const action: Action = {};
      if (base.field === 'command') action.command = fuzzed;
      else if (base.field === 'code') action.code = fuzzed;
      else action.params = { input: fuzzed };
    ...
    if (config?.tools) {
      for (const tool of config.tools) {
        const toolResult = this.scanMCPTool({
          name: tool.name,
          description: tool.description,
          inputSchema: tool.input as Record<string, any>,
        });
        if (!toolResult.safe) {
          for (const f of toolResult.findings) {
            passed++;
            if (!coverage['injection']) coverage['injection'] = { tested: 0, caught: 0 };
  ...
  private _fuzzPayload(payload: string): string {
    const fuzzers = [
      // Case variation
      ...
      // Add quotes
      (s: string) => s.replace(/(\w+)/, '"$1"'),
      // Add path prefix
      (s: string) => s.replace(/^(\w+)/, '/usr/bin/$1'),
      // Double spaces
      (s: string) => s.replace(/\s+/g, '  '),
      // Tab instead of space
      (s: string) => s.replace(/\s+/g, '\t'),
      // Add comment
      (s: string) => s + ' # harmless comment',
      // Env var expansion
      (s: string) => s.replace(/(\/\w+)/, '${HOME}'),
    ];
